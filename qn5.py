...
#------------------------------------------------------------
def forward_prop(input, weights1, weights2, weights3, bias1, bias2, bias3):
    h1 = tf.nn.sigmoid(tf.matmul(input, weights1) + bias1)
    h2 = tf.nn.sigmoid(tf.matmul(h1, weights2) + bias2)
    y = tf.matmul(h2, weights3) + bias3
    return y
...
def main():
    ...
    weights_1 = init_weights(NUM_FEATURES, NUM_HIDDEN_1)
    weights_2 = init_weights(NUM_HIDDEN_1, NUM_HIDDEN_2)
    weights_3 = init_weights(NUM_HIDDEN_2, NUM_CLASSES)
    bias_1 = init_bias(NUM_HIDDEN_1)
    bias_2 = init_bias(NUM_HIDDEN_2)
    bias_3 = init_bias(NUM_CLASSES)

    #Define loss and optimizer
    logits = forward_prop(X, weights_1, weights_2, weights_3, bias_1, bias_2, bias_3)
