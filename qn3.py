...
def main():
    ...
    with tf.Session() as sess:
        ...
        for i in range(epochs):
            ...
            for start, end in zip(range(0, N, batch_size), range(batch_size, N, batch_size)):
                _, loss_val = sess.run([train_op, loss],feed_dict={X: features[start:end], Y: targets[start:end]})
    ...
    np.savetxt("Testing_Accuracy_HN10.csv", test_acc, delimiter=",")
    np.savetxt("Training_Loss_HN10.csv", total_loss, delimiter=",")
    np.savetxt("Time_Per_Epoch_HN10.csv", time_per_epoch, delimiter=",")
    ...
    plt.savefig('Test_Accuracy_HN10.png')
    plt.show()
    sess.close()
